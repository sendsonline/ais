            ...
            <div class="tab-content">
              ...
              <div class="tab-pane" id="updateprofile">
                <form class="form-horizontal" method="post" action="../process/updateprofileprocess.php">
                   ...
                   <?php  include('../includes/connect.php');
                $stud_id = $_SESSION['stud_id'];
                $query = mysqli_query($db,"SELECT * from players LEFT JOIN course ON players.course_id = course.course_id LEFT JOIN category ON players.sports_id = category.cat_id LEFT JOIN sports ON category.sport_id = sports.sport_id LEFT JOIN year ON players.level_id = year.level_id WHERE players.stud_id = '$stud_id'") or die (mysql_error());
                while($row = mysqli_fetch_assoc($query)){?>
            ...
            $g = mysqli_query($db,"SELECT * FROM sports") or die (mysql_error());
              while($rows = mysqli_fetch_assoc($g)){
                      ...
                      ?>
                        ...
                        <option value="<?php echo $row['sport_id'];?>"><?php echo $rows['sports'];?></option>
                  <?php }
                  ...
                  <div class="form-group">
                  <label class="col-xs-2 control-label">Sports Category</label>
                ...
                <div class="form-group">
                 ...
                 <div class="col-sm-8">
                      ...
                      <textarea type="text" class="form-control" name="allergy" value="" required><?php echo $row['allergy']; ?></textarea>
                    </div>
             </div>
 ...
 <div class="form-group">
              ...
              <thead>
                ...
                <tbody>
                  <?php include('../includes/connect.php');
            $id = $_SESSION['stud_id'];
            $query = mysqli_query($db,"SELECT * FROM winners as A LEFT JOIN category as B ON A.cat_id = B.cat_id LEFT JOIN sports as C ON B.sport_id = C.sport_id LEFT JOIN players as D ON B.cat_id = D.sports_id LEFT JOIN sy as E ON A.year_id = E.year_id LEFT JOIN place as F ON A.place_id = F.place_id WHERE D.stud_id = '$id'") or die (mysql_error());
            while($row = mysqli_fetch_assoc($query)){
